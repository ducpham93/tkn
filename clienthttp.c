...
int main(int argc, char *argv[])
{
	...
	hostname = strtok(lineConst,"//"); // find the first double slash
	...
	char host[7] = "HOST: ";
	char misc[5] = "\r\n";
	char https[14] = " HTTP/1.1\r\n";
	
	char *buffer = (char*) calloc (1, sizeof(char)* (strlen(get) + strlen(path) + strlen(https) + 
	...
	if (buffer == NULL) {
		...
		strncpy (buffer, get, strlen(get));
		strncat (buffer, path, strlen(path));
		strncat (buffer, https, strlen(https));
		strncat (buffer, host, strlen(host));
		strncat (buffer, hostname, strlen(hostname));
		strncat (buffer, misc, strlen(misc));
		strncat (buffer, misc, strlen(misc));
