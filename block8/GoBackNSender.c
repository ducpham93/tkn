...
// send   packet
    ...
    if (FD_ISSET(s, &readfds)) {
       ...
       */
      if (crcValid == true && ack->seqNoExpected  != -1 
        && bufferContainsPacket(dataBuffer, ack->seqNoExpected ) == true  )
      {
         lastAckSeqNo =ack->seqNoExpected -1;
         freeBuffer( dataBuffer, getFirstSeqNoOfBuffer(dataBuffer), lastAckSeqNo)  ;
         DataPacket* tmp = getDataPacketFromBuffer(dataBuffer, ack->seqNoExpected) ;
         timerExpiration = tmp->timeout ;
      }
    ...
    if (FD_ISSET(s, &writefds)) {
      while (nextSendSeqNo < lastAckSeqNo + window
        && lastAckSeqNo < (getLastSeqNoOfBuffer(dataBuffer) + 1) ) {
         ...
         *                          like <, >, <=, >=, ==
         */
         DataPacket*  packettmp = getDataPacketFromBuffer(dataBuffer,(nextSendSeqNo-1)) ; 
         timeradd(&timeout,&currentTime,&(packettmp->timeout)) ; 

         DataPacket* tmp = getDataPacketFromBuffer(dataBuffer, getFirstSeqNoOfBuffer(dataBuffer) );
         timerExpiration = tmp->timeout ;
        /* END YOUR TASK */
      }
    }
